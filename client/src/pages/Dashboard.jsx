...
import { toggleAgenda } from "../services/barbero.service";
...
const IconUnlock = () => (
    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" >
        <rect x="3" y="11" width="18" height="11" rx="2" ry="2" />
        <path d="M7 11V7a9 9 0 0114 7.46" />
    </svg>
);
        ...
        <div className="min-h-[calc(100vh-80px)] bg-[#f7f4ef] montserrat-alternates p-5 md:p-10">
            <div className="max-w-4xl mx-auto">
                ...
                <div>
                    ...
                    <h1 className="text-2xl font-black text-[#1e2535]">
                    Hola, {barbero.nombre}
                    </h1>
                    <p className="text-sm text-[#8a8070] mt-0.5">
                ...
                {[
                    ...
                    {
                    label: "Horario",
                    value: `${barbero.sucursal?.horarioApertura} – ${barbero.sucursal?.horarioCierre}`,
                    },
                ].map((item) => (
                    <div
                    key={item.label}
                    className="bg-white rounded-xl border border-[#e8e2d8] px-5 py-4"
                    >
                ...
                <div
                className={`mb-5 rounded-full border px-5 py-4 flex items-center justify-between gap-4 ${agendaAbierta ? "bg-white border-[#e8e2d8]" : "bg-[#1e2535] border-[#1e2535]"}`}
                >
                <div className="flex items-center gap-3">
                    <div
                    className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${agendaAbierta ? "bg-green-50 text-green-700 " : "bg-white/10 text-amber-400"}`}
                    >
                ...
                <button
                    ...
                    agendaAbierta
                        ...
                        : "bg-white text-[#1e2535] border border-gray-200 hover:bg-[#f7f4ef]"
                    }`}
                >
                    {toggling
                    ? "..."
                ...
                {!loading && turnosActivos.length > 0 && (
                    ...
                    {turnosActivos
                        ...
                        <div
                            key={t.id}
                            className="flex items-center gap-5 px-6 py-4 hover:bg-[#faf8f5] transition-colors"
                        >
