...
export default function Navbar() {
    ...
    const { barbero, links, drawerAbierto, cerrarSesion, cerrarDrawer, toggleDrawer } = useNavbar()
            ...
            <nav className="sticky top-0 z-40 bg-slate-900/90 backdrop-blur-md border-b border-white/5 montserrat-alternates">
                <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
                    ...
                    <div className="hidden md:flex items-center gap-8">
                        ...
                        <UserSection barbero={barbero} onLogout={cerrarSesion} />
                    </div>
                    <HamburgerButton abierto={drawerAbierto} onClick={toggleDrawer} />
                </div>
            </nav>
            <MobileDrawer abierto={drawerAbierto} links={links} pathname={pathname} barbero={barbero} onClose={cerrarDrawer} onLogout={cerrarSesion} />
        </>
    )
}
