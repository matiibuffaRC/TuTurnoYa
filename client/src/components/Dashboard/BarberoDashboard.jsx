...
export default function BarberoDashboard() {
  ...
  const handleFecha = (fecha) => {
    setFecha(fecha);
    if (barbero && token) fetchTurnos(barbero.id, fecha, token);
  };
  ...
  const handleUpdateHorarios = async () => {
    ...
    try {
      const updated = await updateHorarios(
        barbero.id,
      ...
      if (updated && !updated.error) {
        ...
        alert(
          "Error al actualizar horarios: " +
            (updated?.error || "Respuesta inválida"),
        );
      }
    } catch (error) {
      console.error("Error updating horarios", error);
      alert("Error al actualizar horarios: " + (error.message || ""));
    }
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] bg-[#f7f4ef] montserrat-alternates p-4 sm:p-6 md:p-10">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5">
          {[
            ...
            { label: "Dirección", value: barbero.sucursal?.direccion || "—" },
            {
              label: "Horario",
              value:
                barbero.sucursal?.horarioApertura &&
                barbero.sucursal?.horarioCierre
            ...
            <div
              key={item.label}
              className="bg-white rounded-xl border border-[#e8e2d8] px-4 sm:px-5 py-4"
            >
        ...
        {/* Agenda abierta */}
        <div
          className={`mb-5 rounded-xl border px-4 sm:px-5 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 ${agendaAbierta ? "bg-white border-[#e8e2d8]" : "bg-[#1e2535] border-[#1e2535]"}`}
        >
          <div className="flex items-center gap-3">
            <div
              className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${agendaAbierta ? "bg-green-50 text-green-700" : "bg-white/10 text-amber-400"}`}
            >
              {agendaAbierta ? <IconUnlock /> : <IconLock />}
            </div>
            <div>
              <p
                className={`text-sm font-bold ${agendaAbierta ? "text-[#1e2535]" : "text-white"}`}
              >
                {agendaAbierta ? "Agenda abierta" : "Agenda cerrada"}
              </p>
              <p
                className={`text-xs mt-0.5 ${agendaAbierta ? "text-[#8a8070]" : "text-white/60"}`}
              >
                {agendaAbierta
          ...
          <button
            ...
            disabled={toggling}
            className={`w-full sm:w-auto text-xs font-bold px-4 py-2.5 rounded-xl transition-colors cursor-pointer disabled:opacity-50 ${agendaAbierta ? "bg-[#1e2535] text-white hover:bg-[#2d3748]" : "bg-white text-[#1e2535] hover:bg-[#f7f4ef]"}`}
          >
            {toggling
              ? "..."
              : agendaAbierta
        ...
        <div className="bg-white rounded-2xl border border-[#e8e2d8] mb-5">
          <div className="flex items-center justify-between px-4 sm:px-6 py-4 sm:py-5 border-b border-[#f0ece4] gap-3">
            <h2 className="text-sm font-black text-[#1e2535] uppercase tracking-wider">
              Agenda
            </h2>
          ...
          {loading && (
            <div className="py-16 text-center text-sm text-[#8a8070]">
              Cargando turnos...
            </div>
          )}
          ...
          {!loading && turnosActivos.length === 0 && (
            <div className="py-16 text-center">
              ...
              <p className="text-xs text-[#8a8070] mt-1">
                Seleccioná otra fecha para ver tu agenda.
              </p>
            </div>
          )}
                  ...
                  >
                    ...
                    <div className="flex items-start sm:items-center gap-3 sm:gap-5">
                      ...
                      <div className="w-12 sm:w-14 shrink-0">
                        <p className="text-base font-black text-[#1e2535]">
                          {turno.hora}
                        </p>
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-[#8a8070] mt-0.5">
                          {turno.servicio?.tipo} · {turno.servicio?.duracion}{" "}
                          min
                        </p>
                      </div>
