...
import { updateHorarios } from "../../services/barbero.service";
...
export default function PanelHorarios({ barbero, onActualizar }) {
  ...
  const guardar = async () => {
    ...
    if (!updated.error) {
      onActualizar({
        ...barbero,
        horariosHabilitados: JSON.stringify(horarios),
      });
      setGuardado(true);
    }
    setGuardando(false);
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#e8e2d8] mb-5">
      <div className="flex items-center justify-between px-6 py-5 border-b border-[#f0ece4]">
        <div>
          <h2 className="text-lg font-black text-[#1e2535] uppercase tracking-wider">
            Mis horarios de trabajo
          </h2>
          <p className="text-xs text-[#8a8070]">
            Horario de la sucursal: {sucursal.horarioApertura} –{" "}
            {sucursal.horarioCierre}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <button
            ...
            className="text-xs font-bold text-[#8a8070] hover:text-[#1e2535] transition-colors cursor-pointer"
          >
            Todos
          </button>
