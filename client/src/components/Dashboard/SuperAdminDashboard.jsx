...
export default function SuperAdminDashboard() {
  ...
  const handleGuardar = async (e) => {
    ...
    if (activeTab === "barberos") {
      if (!formData.sucursalId) {
        setFormError("Seleccioná una sucursal antes de guardar.");
        return;
      }
      if (
        formData.horarioEntrada &&
        formData.horarioSalida &&
        formData.horarioEntrada >= formData.horarioSalida
      ) {
        setFormError(
      ...
      if (!modoEdicion && !formData.password) {
        setFormError("La contraseña es obligatoria para crear un barbero.");
        return;
      }
    }
    ...
    try {
      ...
      setFormError(
        error.message || "Ocurrió un error al guardar. Verificá los datos.",
      );
      console.error(error);
    }
  };
  ...
  const inputCls =
    "w-full bg-[#111827] border border-gray-700 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-amber-500";
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex gap-4 border-b border-gray-800 mb-6 sm:mb-8">
          {["sucursales", "barberos"].map((tab) => (
            <button
              ...
              className={`cursor-pointer pb-4 px-2 text-sm font-bold transition-colors border-b-2 ${activeTab === tab ? "border-amber-400 text-amber-400" : "border-transparent text-gray-500 hover:text-gray-300"}`}
            >
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
          ...
          <button
            ...
            className="cursor-pointer w-full sm:w-auto bg-amber-500 hover:bg-amber-400 text-gray-900 font-bold text-xs px-5 py-2.5 rounded-xl transition-colors shadow-lg shadow-amber-500/20"
          >
            ...
            <table className="w-full text-left text-sm text-gray-400">
              ...
              <tbody className="divide-y divide-gray-800">
                {activeTab === "sucursales"
                  ? sucursales.map((suc) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 text-amber-400/80">
                          {suc.horarioApertura} - {suc.horarioCierre}
                        </td>
                        <td className="px-6 py-4 text-right space-x-3">
                          <button
                            onClick={() => abrirModalEdicion(suc)}
                            className="cursor-pointer text-blue-400 hover:text-blue-300 font-semibold text-xs"
                          >
                            Editar
                          </button>
                          <button
                            onClick={() => handleEliminar(suc.id)}
                            className="cursor-pointer text-red-400 hover:text-red-300 font-semibold text-xs"
                          >
                            Eliminar
                          </button>
                        </td>
                  ...
                  : barberos.map((barb) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 text-amber-400/80">
                          {barb.horarioEntrada} - {barb.horarioSalida}
                        </td>
                        <td className="px-6 py-4 text-right space-x-3">
                          <button
                            onClick={() => abrirModalEdicion(barb)}
                            className="hover:cursor-pointer text-blue-400 hover:text-blue-300 font-semibold text-xs"
                          >
                            Editar
                          </button>
                          <button
                            onClick={() => handleEliminar(barb.id)}
                            className="hover:cursor-pointer text-red-400 hover:text-red-300 font-semibold text-xs"
                          >
                            Eliminar
                          </button>
                        </td>
                ...
                {((activeTab === "sucursales" && sucursales.length === 0) ||
                  (activeTab === "barberos" && barberos.length === 0)) && (
                  <tr>
                    <td
                      colSpan="5"
                ...
                sucursales.map((suc) => (
                  <div key={suc.id} className="px-5 py-4 space-y-1">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span className="shrink-0 text-xs text-gray-600 font-bold">
                        #{suc.id}
                      </span>
                    </div>
          ...
          <div className="bg-[#1f2937] border border-gray-800 rounded-t-3xl sm:rounded-3xl p-5 sm:p-8 w-full sm:max-w-lg max-h-[90dvh] overflow-y-auto shadow-2xl">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg sm:text-xl font-black text-white">
                ...
                {activeTab === "sucursales" ? "Sucursal" : "Barbero"}
              </h3>
              <button
